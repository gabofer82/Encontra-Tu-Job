...
if (isset($_REQUEST['action'])) {
	...
	echo "<script>alert(\"" . var_dump($orden) . "\");</script>";
} else {
...
if ($orden == "altaCandidato") {
	if ($CanAdmin -> validarContrasena($_POST['txtUsrPass'], $_POST['txtUsrRePass'])) {
		...
		echo var_dump($_POST['txtUsrPass']);
		echo var_dump($pass);
	...
	if ($_SESSION['user'] -> getPass() == $passVieja) {
		if ($CanAdmin -> validarContrasena($_POST['txtUsrPass'], $_POST['txtUsrRePass'])) {
			...
			$fecha = $_POST['anhoNac'] . "" . $_POST['mesNac'] . "" . $_POST['diaNac'];
			echo "aca llegamos";
			$retorno = $CanAdmin -> modCandidato($pass, $_POST['txtNombre'], $_POST['txtApellido'], $_POST['radioSexo'], $_POST['slcciudad'], $_POST['slcpais'], $fecha);
